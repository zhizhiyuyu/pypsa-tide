...
#tide 
def convert_tidalplant(ds, lagoon, operation, capacity, area):
    mask = capacity!=0
    time_series = input_0D.time_series(ds)
    eta = ds['tide_elevation']

    eta_to_compute = eta.where(mask).chunk({'time': -1})
    Dt = 240
    cycles = (time_series[-1] + 241)/ (12.42 * 3600)

    da= wrapper_run_simulation(eta_to_compute, area, capacity, time_series, lagoon,operation, Dt, cycles)
    da.attrs["units"] = "MWh/MWp"
    da = da.rename("specific generation")
    return da

def tidalplant(cutout, lagoon=None, operation=None, capacity=None, area=None, **params):
    if isinstance(lagoon, (str, Path)):
        lagoon = get_tidalrangeconfig(lagoon)
    return cutout.convert_and_aggregate(
