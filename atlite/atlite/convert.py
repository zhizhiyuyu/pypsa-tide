...
#tide 
def convert_tidalplant(ds, area, total_capacity):
    # cutout has 100 data in a time chunk
    # reshape the WL into a complete chunk in the time axis
    WL = ds['tide_elevation'].chunk({'time': -1})
    time_series = input_0D.time_series(ds)
    da= wrapper_run_simulation(WL, area, total_capacity, time_series)
    da.attrs["units"] = "MWh/MWp"
    da = da.rename("specific generation")
    return da

def tidalplant(cutout,path, sheet_name, **params):
    
    data = pd.read_excel(path,  sheet_name=sheet_name).rename(columns={'Longitude':'x', 'Latitude':'y'})
    area = cutout.area_list(data, col = 'area')
    total_capacity = cutout.layout_from_capacity_list(data, col = "Project Capacity")
    
    return cutout.convert_and_aggregate(
    convert_func=convert_tidalplant,
    area=area,
...
def tidalrange(cutout, return_capacity=False):
    ...
    if return_capacity:
        ...
        da.attrs["units"] = "MW/km^2"
        da = da.rename("capacity per square meter") 
...
def wave(cutout, generator,  **params):
    ...
    Evaluates the significant wave height (Hs) and eave peak period (Tp) and 
    assesses the power output with the chosen power matrix for each time step and grid cell
    """
