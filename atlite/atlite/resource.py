...
TIDALRANGE_DIRECTORY = RESOURCE_DIRECTORY / "tidalrange"
...
def get_tidalrangeconfig(lagoon):
    """Load the 'lagoon'.yaml file from local disk to provide the system efficiencies.

    Parameters
    ----------
    installation : str
        Name of tidal plant lagoon. Must correspond to name of one of the files
        in resources/tidalrange

    Returns
    -------
    config : dict
        Config with details on the lagoon.
    """
    assert isinstance(lagoon, (str, Path))

    if isinstance(lagoon, str):
        if not lagoon.startswith("lagoon_"):
            return lagoon 
        if not lagoon.endswith(".yaml"):
            lagoon_path = tidalranges[lagoon.replace(".yaml", "")]
    elif isinstance(lagoon, Path):
        lagoon_path = lagoon
        
    with open(lagoon_path, "r") as f:
        conf = yaml.safe_load(f)
    return conf
...
def get_wecgeneratorconfig(generator):
    ...
    """
    assert isinstance(generator, (str, Path))
    if isinstance(generator, str):
        if not generator.endswith(".yaml"):
            generator_path = wecgenerators[generator.replace(".yaml", "")]
    elif isinstance(generator, Path):
...
tidalranges = arrowdict({p.stem: p for p in TIDALRANGE_DIRECTORY.glob("*.yaml")})
