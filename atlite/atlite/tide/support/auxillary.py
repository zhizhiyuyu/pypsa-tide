...
def determine_capital_cost_per_KW(capacity, tidal_range, lagoon_factor=0.75, efficiency=0.5, capacity_factor=0.20):
    turbine_cost, caisson_cost = 1000, 1000

    grav, dens = 9.807, 1025
    head_difference_factor = 1.5

    DH = tidal_range * head_difference_factor
    area_required = capacity_factor * capacity / (
    (0.5 * grav * dens * DH * DH / 1e6 * 0.000277778) * efficiency / 12.42)
    print("Area required for scheme:", area_required / 1e6, "km2")
    print("Estimated annual output: ",
          (0.5 * grav * dens * area_required * DH * DH / 1e6 * 0.000277778) * efficiency * 365 * 24 / 12.42, "MWh")
