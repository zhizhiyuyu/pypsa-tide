...
def hill_chart_parametrisation_p(h, turbine_specs):
...
def hill_chart_parametrisation_h(h, turbine_specs):
    # f_g = grid frequency, g_p = generator poles,
    # t_cap = Turbine capacity, h = head difference, dens = water density
    turb_sp = 2 * 60 * turbine_specs["f_g"] / turbine_specs["g_p"]

    # Step 1: Calculate Hill Chart based on empirical equations
    n_11 = turb_sp * turbine_specs["t_d"] / np.sqrt(h)
    ...
    q = q_11 * (turbine_specs["t_d"] ** 2) * np.sqrt(h)
    h_efficiency = -0.0019 * n_11 + 1.2461
    # h_efficiency  = 1
    p1 = turbine_specs["dens"] * turbine_specs["g"] * q * h / (10 ** 6)
    # Step 2 - Adjust Curve according to capacity
    if h < turbine_specs["h_cap"]:  # 97.25% Gearbox efficiency
        p2 = p1 * 0.9725 * h_efficiency
    else:
        n_11 = turb_sp * turbine_specs["t_d"] / np.sqrt(turbine_specs["h_cap"])
        q_11 = 0.0166 * n_11 + 0.4861
        h_efficiency = -0.0019 * n_11 + 1.2461
        q = q_11 * (turbine_specs["t_d"] ** 2) * np.sqrt(turbine_specs["h_cap"])
        p2 = turbine_specs["dens"] * turbine_specs["g"] * q * turbine_specs["h_cap"] / \
...
def turbine_parametrisation(h, turbine_specs):
    if turbine_specs["options"] == 0:
        p, q = hill_chart_parametrisation_p(h, turbine_specs)
    elif turbine_specs["options"] == 1:
        p, q = hill_chart_parametrisation_h(h, turbine_specs)
    else:
...
def turbine_generation(h, ramp_f, N_t, turbine_specs):
    q_t = - np.sign(h) * ramp_f  * N_t * turbine_parametrisation(abs(h), turbine_specs)[1]
    p = ramp_f * N_t * turbine_parametrisation(abs(h), turbine_specs)[0] * turbine_specs["eta"][0]

    return q_t, p


def gate_sluicing(h, ramp_f, N_s, q_s0, sluice_specs, flux_limiter=0.2):
    temp = ramp_f * N_s * sluice_specs["c_d"] * sluice_specs["a_s"] * math.sqrt(2 * sluice_specs["g"] * abs(h))
...
def turbine_sluicing(h, ramp_f, N_t, q_t0, sluice_specs, turbine_specs, flux_limiter=0.2):
    temp = ramp_f * N_t * sluice_specs["c_t"] * (math.pi * (turbine_specs["t_d"] / 2) ** 2) * \
...
def ramping_turbine(turbine_transition, t, duration = 0.2):
    return (turbine_transition["final"]-turbine_transition["initial"]) * \
           min((t-turbine_transition["start_t"])/duration, 1.0) + turbine_transition["initial"]
